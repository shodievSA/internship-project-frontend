...
import {
	useMyProductivity,
	useMemberProductivity,
} from "../hooks/useMemberProductivity";
...
export function TeamMemberProductivityModal({ member, isOpen, onClose }) {
	...
	const { currentMemberId, currentMemberRole } = useProject();
	...
	// Check if user has permission to view other members' productivity
	const canViewOtherProductivity =
		currentMemberRole === "admin" || currentMemberRole === "manager";
	const hasPermission = isOwnProductivity || canViewOtherProductivity;
	...
	// Use appropriate hook based on whether viewing own or other member's productivity
	const {
		data: ownData,
		loading: ownLoading,
		error: ownError,
	} = useMyProductivity(projectId, selectedSprintId, {
	...
	const {
		...
		error: memberError,
	} = useMemberProductivity(projectId, member?.id, selectedSprintId, {
		shouldFetch: isAuthenticated && !isOwnProductivity && hasPermission,
	});

	// Use the appropriate data based on what we're viewing
	const { data, loading, error } = isOwnProductivity
		? { data: ownData, loading: ownLoading, error: ownError }
	...
	const handleSprintChange = (sprintId) => {
		// Only allow sprint changes if user has permission
				...
				{!isAuthenticated ? (
					<div className="flex flex-col items-center justify-center py-16 text-center">
						...
						<p className="text-gray-600 dark:text-gray-400 max-w-sm text-sm leading-relaxed">
							You must be signed in to view productivity data.
							Please sign in to access your metrics.
						</p>
					</div>
				) : !hasPermission ? (
					<div className="flex flex-col items-center justify-center py-16 text-center">
						...
						<p className="text-gray-600 dark:text-gray-400 max-w-sm text-sm leading-relaxed">
							{!isOwnProductivity && canViewOtherProductivity
								? "You don't have permission to view this member's productivity data."
								: "Productivity data is only available for your own account. Click on your profile card to view your metrics."}
						</p>
					</div>
				) : (
					<>
						...
						<div className="flex mb-4">
							...
							{!isOwnProductivity && canViewOtherProductivity && (
								<div className="ml-4 flex items-center">
									<div className="px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
										<span className="text-xs font-medium text-blue-700 dark:text-blue-300">
											Viewing {member.name}'s data
										</span>
									</div>
								</div>
							)}
							...
							<div className="flex flex-col items-center justify-center py-16 text-center">
								...
								<p className="text-gray-600 dark:text-gray-400 max-w-sm text-sm leading-relaxed">
									...
									error.includes("Forbidden")
										...
										: error.includes("404") ||
										  error.includes("Not Found")
										? "The requested member was not found in this project."
									...
									{productivityData?.timeTracking?.timePerTask
										...
										<div className="px-8 py-16 text-center">
											...
											<p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto text-sm leading-relaxed">
												Start tracking your time on
												tasks to see your time
														...
														<div className="text-lg font-semibold text-gray-900 dark:text-white">
															{(() => {
																...
																return mostTimeSpent
																	.taskTitle
																	.length > 15
																	? mostTimeSpent.taskTitle.substring(
																			0,
																			15
																	  ) + "..."
