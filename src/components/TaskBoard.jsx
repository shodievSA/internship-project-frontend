...
function DroppableColumn({ id, children }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className="w-full rounded-2xl shadow-md transition-colors bg-white dark:bg-gray-800 flex flex-col max-h-[70vh] min-w-[280px]"
    >
      {children}
    </div>
  );
...
function TaskBoard({ tasks = [] }) {
  ...
  const { updateTaskInContext, setTasks } =
    useProject();
          ...
          {STATUS_COLUMNS.map((col) => {
            ...
            return (
              <DroppableColumn key={col.key} id={col.key}>
                {/* Column Header */}
                <div
                  className={`${"rounded-t-2xl px-4 py-3 flex items-center justify-between"} ${col.color
                    } dark:opacity-90`}
                >
                ...
                >
                  <div className="flex-1 min-h-0 flex flex-col gap-4 px-4 py-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent">
                    ...
                    {columns[col.key]?.length > 0 &&
                      placeholderIndex === columns[col.key].length && (
                        <div
                          style={{
                            height: 112,
                            marginBottom: 16,
