				...
				<div className="flex items-center justify-center h-64">
					<div className="text-center">
						...
						<p className="text-gray-500 dark:text-gray-400 text-sm max-w-sm">
							There are no tasks with priority levels for this
							project yet. Priority data will appear here once
							tasks are created and assigned priority levels.
						</p>
					</div>
				</div>
			</div>
	...
	const chartData = [
		{
			...
			percentage:
				data.priorities.find((p) => p.level === "high")?.percentage ||
				0,
			icon: "↑",
		...
		},
		{
			name: "Middle",
			count:
				data.priorities.find((p) => p.level === "middle")?.count || 0,
			percentage:
				data.priorities.find((p) => p.level === "middle")?.percentage ||
				0,
			icon: "=",
					...
					>
						...
						<Bar dataKey="count" radius={[4, 4, 0, 0]}>
							{chartData.map((entry, index) => {
								...
								else if (entry.name === "Middle")
									gradientId = "middleGradient";
