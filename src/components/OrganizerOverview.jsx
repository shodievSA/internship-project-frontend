			...
			>
				<div className="flex justify-between">
					...
					<div className="flex items-center gap-x-5">
						<span className={`${taskPriorityColors[task.priority]} text-xs px-3 
						rounded-full py-0.5`}>
							{ task.priority }
						</span>
						<div className="flex items-center gap-x-2">
							<Clock className="w-4 h-4" />
							<span className="text-sm mt-0.5">{ formatIsoDate(task.deadline) }</span>
						</div>
					</div>
				</div>
				<div className="flex justify-between">
	...
	function returnNotification(notification) {
		...
		return (
			...
			>
				...
				<div className="flex justify-between">
					<span className="text-sm">{ formatIsoDate(notification.createdAt) }</span>
			...
			>
				...
				<div className="flex justify-between">
					<div className="flex gap-x-3 text-sm">
						<span>In: <span className="font-medium">{ task.from.projectName }</span></span>
						<span>Submitted by: <span className="font-medium">{ task.assignedTo.name }</span></span>
					</div>
					<div className="opacity-0 group-hover/item:opacity-100 transition-[opacity]
					duration-300">
						<ChevronRight className="w-5 h-5 text-neutral-500 dark:text-neutral-300" />
					...
					tasksDueToday.length > 0 ? (
						tasksDueToday.length > 3 ? (
							<div className="flex flex-col gap-y-3">
								<div className="flex flex-col gap-y-3">
									{ tasksDueToday.slice(0, 3).map((task) => returnTask(task)) }
								</div>
								<Button 
									size="md" 
									variant="secondary"
					...
					</div>					
					{
						tasksDueTomorrow.length > 0 ? (
							tasksDueTomorrow.length > 3 ? (
								<div className="flex flex-col gap-y-3">
									<div className="flex flex-col gap-y-3">
										{ tasksDueTomorrow.slice(0, 3).map((task) => returnTask(task))}
									</div>
									<Button 
										size="md"
										variant="secondary"
								...
								<div className="flex flex-col gap-y-3">
									{ tasksDueTomorrow.map((task) => returnTask(task))}
								</div>
							)
						) : (
					...
					</div>					
					{
						tasksDueThisWeek.length > 0 ? (
							tasksDueThisWeek.length > 3 ? (
								<div className="flex flex-col gap-y-3">
									<div className="flex flex-col gap-y-3">
										{ tasksDueThisWeek.slice(0, 3).map((task) => returnTask(task)) }
									</div>
									<Button 
										size="md"
										variant="secondary" 
								...
								<div className="flex flex-col gap-y-3">
									{ tasksDueThisWeek.map((task) => returnTask(task)) }
								</div>
							)
						) : (
					...
					</div>
					{
						tasksForReview.length > 0 ? (
							tasksForReview.length > 3 ? (
								<div className="flex flex-col gap-y-3">
									<div className="flex flex-col gap-y-3">
										{ 
											tasksForReview
											.slice(0, 3)
											.map((taskForReview) => returnTaskForReview(taskForReview)) 
										}
									</div>
									<Button 
										size="md" 
								...
								<div className="flex flex-col gap-y-3">
									{ tasksForReview.map((taskForReview) => returnTaskForReview(taskForReview)) }
								</div>
							)
						) : (
					...
					</div>					
					{
						newNotifications.length > 0 ? (
							newNotifications.length > 3 ? (
								<div className="flex flex-col gap-y-3">
									<div className="flex flex-col gap-y-3">
										{ 
											newNotifications
											.slice(0, 3)
											.map((notification) => returnNotification(notification)) 
										}
									</div>
									<Button 
										size="md" 
								...
								<div className="flex flex-col gap-y-3">
									{ newNotifications.map((notification) => returnNotification(notification)) }
								</div>
							)
						) : (
