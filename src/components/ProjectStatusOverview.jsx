...
function ProjectStatusOverview({ sprintId = null }) {
	...
	const statusColors = {
		...
		ongoing:
			"linear-gradient(135deg, #3B82F6 0%, #1D4ED8 50%, #1E40AF 100%)", // Bright Blue
		...
		overdue:
			"linear-gradient(135deg, #FFA500 0%, #FF8C00 50%, #FF6B35 100%)", // Orange/Amber

		rejected:
			"linear-gradient(135deg, #FF6B6B 0%, #FF8E8E 50%, #E74C3C 100%)", // Coral Red
	};
				...
				<div className="flex items-center justify-center h-64 px-6 pb-6">
					<div className="text-center">
						...
						<p className="text-gray-500 dark:text-gray-400 text-sm max-w-sm">
							There are no tasks with status information for the
							sprint yet. Status data will appear here once tasks
	...
	const chartData = Object.entries(displayData.statusDistribution).map(
		([status, count]) => {
			...
			return {
				...
				originalStatus: status, // Keep original status for color mapping
				value: count,
				color: mappedColor,
				percentage: Math.round(
					(count / displayData.totalWorkItems) * 100
				),
			};
		}
	);
						...
						<PieChart>
							...
							>
								{chartData.map((entry, index) => {
									...
									if (entry.originalStatus === "ongoing")
										gradientId = "ongoingGradient";
									else if (entry.originalStatus === "closed")
										gradientId = "closedGradient";
									else if (
										entry.originalStatus === "underReview"
									)
										gradientId = "underReviewGradient";
									else if (entry.originalStatus === "overdue")
										gradientId = "overdueGradient";
									else if (
										entry.originalStatus === "rejected"
									)
