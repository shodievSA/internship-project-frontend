...
function GroupEmailModal({ projectId, teamMembers, closeModal }) {
		...
		<Modal title="Send Email to Team Members" size="lg">
			...
			>
				<div className="flex flex-col gap-y-3">
					...
					<div
						className="p-4 rounded-md border-[1px] dark:border-neutral-800 max-h-52 overflow-y-auto 
							...
							return (
								<div className="flex gap-x-4 items-center">
									<button
										...
										value={member.id}
										className={`w-5 h-5 ${
											recipients.includes(member.id)
												? "bg-pink-500 border-pink-500 text-white"
												: "dark:border-neutral-600 border-slate-400"
										} border-[1px] border-neutral-600 rounded-md cursor-pointer 
					...
					<p className="text-sm">
						Selected: {recipients.length} / {teamMembers.length}
					</p>
				</div>
				<InputField
