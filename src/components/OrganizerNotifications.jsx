...
function OrganizerNotifications({ newNotifications }) {
	...
	if (newNotifications.length === 0) return <EmptyState message={"You're all caught up - not a single update in sight!"} />
	...
	return (
		<div>
			<div className="flex flex-col gap-y-5">
				<div>
					<h1>
						You have {newNotifications.length} {newNotifications.length > 1 ? "new notifications" : "new notification"}
					</h1>
				</div>
				<div className="flex flex-col gap-y-5">
					{
						newNotifications.map((notification) => {
							return (
								...
								>
									<h1 className="text-lg font-medium">{notification.title}</h1>
