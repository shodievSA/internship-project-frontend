...
function Comments() {
  ...
  if (!commentsFetched)
    return (
      <LoadingState message={"Hang on - the comments are on their way!"} />
    );
  ...
  return (
    <div className="h-full flex flex-col px-8 pt-4">
      ...
      <div className="flex flex-col flex-1 min-h-0 gap-y-4">
        {comments.length === 0 ? (
          <EmptyState
            message={"No comments yet - be the first to break the silence!"}
          />
        ) : (
          <div
            ref={chatWindowRef}
...
function CurrentUserComment({
  ...
  function isUpdateValid() {
    return newMessage.trim() !== "" && newMessage.trim() !== comment.message;
  }
