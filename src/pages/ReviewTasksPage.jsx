...
function ReviewTasksPage() {
	...
	const reviewTasks = useMemo(() => {
		...
		return tasks.filter((task) => {
			return (
				task.assignedBy.id === currentMemberId &&
			...
			if (element) {
				...
				setTimeout(() => {
					element.classList.add(
						"animate-task-ping-light",
						"dark:animate-task-ping-dark",
					);
	...
	if (reviewTasks.length === 0)
		return (
			<EmptyState
				message={
					"You’re all caught up - no tasks to review for now. Time to kick back and enjoy the calm!"
				}
			/>
		);
