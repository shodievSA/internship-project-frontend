...
const sprintService = {
	createSprint: async (projectId, sprint) => {
		...
		if (!response.ok) {
			...
			throw new Error(error.error || "Failed to create new sprint");
		}
	...
	deleteSprint: async (projectId, sprintId) => {
		...
		if (!response.ok) {
			const error = await response.json();
			throw new Error(error.error || "Failed to delete the sprint");
		}
	...
	getSprintDetails: async (projectId, sprintId) => {
		...
		if (!response.ok) {
			const error = await response.json();
			throw new Error(error.error || "Failed to load sprint tasks!");
		}
