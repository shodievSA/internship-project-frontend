...
export async function startTimer(taskId) {
	...
	if (!res.ok) {
		...
		throw new Error(
			error.error || error.message || "Failed to start timer"
		);
	}
...
export async function createManualEntry(taskId, startTime, endTime, note) {
	...
	if (!res.ok) {
		const error = await res.json().catch(() => ({}));
		throw new Error(
			error.error || error.message || "Failed to create manual time entry"
		);
	}
	return await res.json();
}
...
export async function fetchTimerStatus() {
	...
	if (!res.ok) {
		const error = await res.json().catch(() => ({}));
		throw new Error(
			error.error || error.message || "Failed to fetch timer status"
		);
	}
	return await res.json();
}
...
export async function fetchTimerStats(params) {
	...
	if (!res.ok) {
		const error = await res.json().catch(() => ({}));
		throw new Error(
			error.error || error.message || "Failed to fetch timer stats"
		);
	}
	return await res.json();
}
